...
class TupleBundleCrossProduct(seeds: Seq[Long] = (0l until 10l).toSeq)
  ...
  with LazyLogging
{
  ...
  def rewrite(db: Database, queryRaw: Operator): (Operator, Seq[String], MetadataT) =
  {
    ...
    val (compiled, nonDeterministicColumns) = compileFlat(query, db)
    query = compiled
  ...
  def compileFlat(query: Operator, db:Database): (Operator, Set[String]) =
  {
    ...
    query match {
      ...
      case Project(columns, oldChild) => {
        val (newChild, nonDeterministicInput) = compileFlat(oldChild, db)
      ...
      case Join(lhsOldChild, rhsOldChild) => {
        val (lhsNewChild, lhsNonDeterministicInput) = compileFlat(lhsOldChild, db)
        val (rhsNewChild, rhsNonDeterministicInput) = compileFlat(rhsOldChild, db)
...
//          val shardedChild = convertFlatToLong(newChild, oldChild.columnNames, nonDeterministicInput)
      ...
      case View(_, query, _) =>  compileFlat(query, db)
